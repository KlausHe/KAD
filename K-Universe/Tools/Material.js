...
function clear_cl_Material() {
  //------clean Data
  Data_Material.matSortList = new Set(Object.entries(Data_Material.Materials).map((arr) => {
    return arr[1].matSort
  }))
  Data_Material.matSortList = [...Data_Material.matSortList].sort()
  Data_Material.matZustandList = new Set(Object.entries(Data_Material.Materials).map((arr) => {
  ...
  for (let i = 0; i < materialOptions.headerList.length; i++) {
    ...
    for (let n = 0; n < materialOptions.matList.length; n++) {
      ...
      tableAddCell(row, {
        ...
        get ui() {
          if (listItem == "matSort") return {
            title: Data_Material.matSortList[value]
          }
          if (listItem == "matZustand") return {
...
function materialSearchInput() {
  ...
  let search = (val == "") ? "" : val.toLowerCase().trim().split(/[*^\s]/g);
