...
function clear_cl_Pythagoras() {
  pythoOptions.vals = [];
  for (let i = 0; i < 5; i++) {
    dbID(`idVin_Pytho_${i}`).value = (pythoOptions.inputState.includes(i)) ? pythoOptions.valsOrig[i] : "";
  }
  pythoShowError();
  calcPytho();
};
...
function calcPytho() {
  ...
  pythoOptions.vals[A] = numberFromInput(`idVin_Pytho_${A}`, null, true);
  pythoOptions.vals[B] = numberFromInput(`idVin_Pytho_${B}`, null, true);
...
function drawPytho() {
  ...
  if ((pythoOptions.vals[0] / pythoOptions.vals[1]) > (pythoOptions.height / pythoOptions.width)) { //--> "Y" fixed length, Scale "X"
  ...
  } else if ((pythoOptions.vals[0] / pythoOptions.vals[1]) < (pythoOptions.height / pythoOptions.width)) { //--> "X" fixed length
