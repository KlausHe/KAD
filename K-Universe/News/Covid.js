...
function covidSetDates(loc, utc = null) {
	...
	if (loc == "A") {
		if (utc === null) {
			...
			btnTime = utilsDate(covidData.dates.A);
		}
	} else if (loc == "B") {
		if (utc === null) {
			...
			btnTime = utilsDate(covidData.dates.B);
		}
	}
...
function createCovidTable() {
	...
	for (let [key, val] of Object.entries(covidData.data)) {
		...
		if (vals.infected !== 0) {
			// only show countries with infections
			covidList.push({
				country: key,
				...vals,
			});
	...
	if (!covidData.showTable) {
		...
		for (let i = 0; i < covidListSorted.length; i++) {
			...
			covidOptions.graphTypes.forEach((lbl, index) => {
				...
				text = `${utilsNumber(text, { decimals: lbl.tableText[0], indicator: true })}${lbl.tableText[1]}`;
				tableAddCell(row, {
				...
				yAxes: [
					{
						...
						ticks: {
							callback: (value) => {
								if (value >= 1000000) {
									return utilsNumber(Math.floor(value / 10000) / 100) + "M";
								} else if (value >= 1000) {
									return utilsNumber(Math.floor(value / 10) / 100) + "T";
								}
								return utilsNumber(value);
							},
					...
					},
					{
						...
						ticks: {
							callback: (value) => {
								return `${utilsNumber(value, { decimals: 2 })}%`;
							},
							...covidOptions.ticksStyle,
						},
						...covidOptions.gridLinesStyle,
...
function covidRefreshGraph() {
	...
	covidGraph.data.labels = covidData.data[covidData.selectedCountry].reduce((result, data) => {
		if (data.dateUTC >= startDate && data.dateUTC <= endDate) {
			result.push(utilsDate(data.dateUTC));
		}
		return result;
	}, []);
...
function covidColorGraph() {
	if (covidGraph != null) {
		...
		covidGraph.options.scales.xAxes[0].gridLines.color = utilsColor.formatAsCSS([...lCol, 0.2], "HSL");

		for (let i = 0; i < covidOptions.graphTypes.length; i++) {
			const alpha = valueMapping(i, 0, covidOptions.graphTypes.length - 1, 0.8, 0.2, true);
			covidGraph.options.scales.yAxes[i].gridLines.color = utilsColor.formatAsCSS([...lCol, alpha], "HSL");
