...
const caIB = new p5((c) => {
  c.setup = function() {
    c.canv = c.createCanvas(ibhaluniOptions.width, ibhaluniOptions.height);
...
class IbhaluniObj {
  constructor(id) {
    ...
    this.x = Math.random() * (ibhaluniOptions.width - this.xSize) + this.xSize / 2;
    this.y = Math.random() * ibhaluniOptions.width * 1 + ibhaluniOptions.height;
    this.popping = false;
  ...
  check() {
    if (this.y < -(this.ySize * 2)) {
      this.resetBalloon();
      ibhaluniOptions.score--
    } else if (this.falling && (this.y > ibhaluniOptions.height + this.ySize)) {
      this.resetBalloon();
    };
  };

  resetBalloon() {
    this.y = Math.random() * ibhaluniOptions.width + ibhaluniOptions.height;
    this.popping = false;
    this.poppingCounter = 0;
    this.poppingScl = 1;
    this.falling = false;
