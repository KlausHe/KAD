...
function linahaCreateButtons() {
  ...
  for (let y = 0; y < rows; y++) {
    const rowDiv = tableAddCell(null, {
...
function linahaShowData(domID, optionsID, dataIndex) {
  ...
  switch (optionsID) {
    ...
    case 3: // population text
      ...
      if (value >= 1000000) {
        retText = (Math.floor(value / 10000) / 100).toLocaleString() + " M"
      } else if (value >= 1000) {
        retText = (Math.floor(value / 10) / 100).toLocaleString() + " T"
      } else {
        retText = value.toLocaleString();
      };
      break;
    case 4: // area text
      const area = obj.area || Math.floor(Math.random() * 20000) * 100
      retText = area.toLocaleString() + " km<sup>2</sup>";
      break;
    default:
      retText = "...";
  }
...
function linahaAnswered(index) {
  ...
  if (linahaOptions.answered) { // answer is chosen
    enableBtn("idBtn_linahaMap", true);
    const isCorrect = (index == linahaOptions.answerIndex);
    linahaOptions.correctRounds += isCorrect ? 1 : 0;
    if (isCorrect) {
      dbIDStyle(`idBtn_child_linahaAnswers_${index}`).backgroundColor = colorReturnFormat(globalValues.colors.elements.btnPositive, {
        type: "hsl",
        text: true
      });
    } else {
      dbIDStyle(`idBtn_child_linahaAnswers_${index}`).backgroundColor = colorReturnFormat(globalValues.colors.elements.btnNegative, {
        type: "hsl",
        text: true
      });
      dbIDStyle(`idBtn_child_linahaAnswers_${linahaOptions.answerIndex}`).backgroundColor = colorReturnFormat(globalValues.colors.elements.btnPositive, {
        type: "hsl",
        text: true
      });
    }
