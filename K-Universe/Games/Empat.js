 ...
 function empatCheckWinner(cell) {
   ...
   //check Diagonal\\\\\
   for (let i = 0; i < empatOptions.cols - 3; i++) { //from left to right
     for (let j = 0; j < empatOptions.rows - 3; j++) { //from top to bottom
       let tempArr = [];
       for (let n = 0; n < 4; n++) {
   ...
   //check Diagonal ////
   for (let i = 0; i < empatOptions.cols - 3; i++) { //from left to right
     for (let j = empatOptions.rows - 1; j >= 3; j--) { //from top to bottom
       let tempArr = [];
       for (let n = 0; n < 4; n++) {
         tempArr.push(empatOptions.cells[i + n][j - n]);
       };
 ...
 class EmpatPlayer {
   ...
   update() {
     ...
     if (this.checkLerpFinished(0) && this.dropped) { //sliding is finished and dropped is pressed
       this.sliding = false;
       this.dropping = true;
     }
     if (this.checkLerpFinished(1) && !this.sliding && this.dropped) { //sliding is finished
       this.dropping = false;
     }
   ...
   moving() {
     if (this.sliding && !this.dropping) { // lerp in x direction
       this.curPos[0] = Math.floor(caEM.lerp(this.curPos[0], this.targetPos[0], empatOptions.speed)); // (start, finish, speed)
     } else if (!this.sliding && this.dropping) { // lerp in y direction
