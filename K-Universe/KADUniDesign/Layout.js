const contentLayout = {
  ...
  defaultPage: globalValues.hostDebug ? "cl_Lotto" : "Universe"
}
...
function layoutCreateContentlayoutList() {
  contentLayout.navContent.Universe = Object.keys(contentGrid).filter((key) => {
...
function layoutCreateGridLayout(layoutName) {
  ...
  if (layoutName === "Clear") { // used in backgroundAnimations to clear all Tiles
    data.contentList = [];
  } else if (layoutName.includes("cl_")) { // fullscreen-subgrid
    ...
    data.contentList = nuncDiscipuli.checkLogin ? [...contentLayout.navContent[layoutName]] : [...contentLayout.navContent[layoutName]].filter((content) => {
      ...
      while (notPlaced) {
        for (let r = 0; r < rowLength; r++) {
          ...
          if (data.gridArray[indexR] !== undefined || Math.floor((indexR + contWidth - 1) / rowLength) != indexRow) {
            ...
            if (!notPlaced) { // if  true, push to Array at these places!
              for (let x = 0; x < contWidth; x++) {
                for (let y = 0; y < contHeight; y++) {
                  const index = indexR + x + (y * rowLength);
