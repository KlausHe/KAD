...
(function(root, factory) {
   ...
   */
  let hasMoment = typeof moment === 'function',
    ...
    hasID = function(el, cn) {
      return (' ' + el.id + ' ').indexOf(' ' + cn + ' ') != -1;
    },
    ...
    compareDates = function(a, b) {
      ...
      let _a = new Date(a.getTime());
      let _b = new Date(b.getTime());
      setToStartOfDay(_a);
      setToStartOfDay(_b);
    ...
    renderHead = function(opts) {
      ...
      if (opts.showWeekNumber) {
        arr.push('<th scope="col"><abbr>KW</abbr></th>');
      }
      for (i = 0; i < 7; i++) {
    ...
    renderTimePicker = function(num_options, selected_val, select_id, display_func, increment_by) {
      increment_by = increment_by || 1;
      let to_return = '<td><select class="pika-select" id="' + select_id + '">';
      for (let i = 0; i < num_options; i += increment_by) {
    ...
    renderTime = function(hh, mm, ss, opts) {
      let to_return = '<table cellpadding="0" cellspacing="0" class="pika-time"><tbody><tr>' +
        (opts.timeLabel != null ? '<td class="pika-time-label">' + opts.timeLabel + '</td>' : '') +
        renderTimePicker(24, hh, 'pika-select-hour', (i) => {
            return (i < 10) ? "0" + i : i;
          },
      ...
      if (opts.showMinutes) {
        to_return += '<td>:</td>' +
          renderTimePicker(60, mm, 'pika-select-minute', (i) => {
            return (i < 10) ? "0" + i : i;
      ...
      if (opts.showSeconds) {
        to_return += '<td>:</td>' +
          renderTimePicker(60, ss, 'pika-select-second', (i) => {
            return (i < 10) ? "0" + i : i;
    ...
    Pikaday = function(options) {
      ...
      self._onMouseDown = function(e) {
        ...
        if (!hasClass(target, 'is-disabled')) {
          if (hasClass(target, 'pika-button') && !hasClass(target, 'is-empty') && !hasClass(target.parentNode, 'is-disabled')) {
            ...
            let prevDate = self._d || opts.defaultDate;
            ...
            if (opts.bound) {
              sto(function() {
                if (opts.autoClose) { //&& !opts.showTime
                  self.hide();
                }
      ...
      self._onChange = function(e) {
        ...
        } else if (hasID(target, 'pika-select-hour')) {
          self.setTime(target.value);
        } else if (hasID(target, 'pika-select-minute')) {
          self.setTime(null, target.value);
        } else if (hasID(target, 'pika-select-second')) {
      ...
      self._onInputBlur = function() {
        ...
        if (opts.autoClose && !self._c) { //&& !opts.showTime
  ...
  Pikaday.prototype = {
    ...
    config: function(options) {
      ...
      opts.autoClose = (!!opts.showTime) ? false : !!opts.autoClose;
    ...
    adjustDate: function(sign, days) {
      ...
      let difference = parseInt(days) * 24 * 60 * 60 * 1000;
    ...
    draw: function(force) {
      ...
      if (opts.showTime) {
        let prevDate = this._d || this._o.defaultDate;
    ...
    adjustPosition: function() {
      let field, pEl, width, height, viewportWidth, viewportHeight, scrollTop, left, top, clientRect, leftAligned, bottomAligned;
    ...
    render: function(year, month, randId) {
      ...
      let minDate_date = opts.minDate ? new Date(opts.minDate.getFullYear(), opts.minDate.getMonth(), opts.minDate.getDate()) : null;
      let maxDate_date = opts.maxDate ? new Date(opts.maxDate.getFullYear(), opts.maxDate.getMonth(), opts.maxDate.getDate()) : null;
